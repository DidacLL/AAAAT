...
function toTemplate(row: TemplateRow): CvTemplateRecord {
  return cvTemplateRecordSchema.parse({
    ...
    sections: parseJson(row.compositionJson, (value) => cvTemplateSectionSchema.array().parse(value), "Stored CV template composition is invalid."),
    createdAt: row.createdAt,
    updatedAt: row.updatedAt,
  });
}
function readTemplates(database: DatabaseSync): CvTemplateRecord[] {
  const rows = database.prepare(`SELECT id, name, language, composition_json AS compositionJson,
                                        created_at AS createdAt, updated_at AS updatedAt
                                   FROM cv_templates ORDER BY name COLLATE NOCASE, id`)
    .all() as unknown as TemplateRow[];
  return rows.map(toTemplate);
}
function requireTemplate(database: DatabaseSync, id: string): CvTemplateRecord {
  const row = database.prepare(`SELECT id, name, language, composition_json AS compositionJson,
                                       created_at AS createdAt, updated_at AS updatedAt
                                  FROM cv_templates WHERE id = ?`).get(id) as unknown as TemplateRow | undefined;
...
function toWorking(row: WorkingRow): WorkingCvRecord {
  return workingCvRecordSchema.parse({
    ...
    sections: parseJson(row.compositionJson, (value) => workingCvRecordSchema.shape.sections.parse(value), "Stored Working CV composition is invalid."),
    createdAt: row.createdAt,
    updatedAt: row.updatedAt,
  });
}
function readWorkingCvs(database: DatabaseSync): WorkingCvRecord[] {
  const rows = database.prepare(`SELECT id, title, language, source_template_id AS sourceTemplateId,
                                        candidature_id AS candidatureId, composition_json AS compositionJson,
                                        created_at AS createdAt, updated_at AS updatedAt
                                   FROM working_cvs ORDER BY updated_at DESC, id DESC`)
...
function requireWorkingCv(database: DatabaseSync, id: string): WorkingCvRecord {
  const row = database.prepare(`SELECT id, title, language, source_template_id AS sourceTemplateId,
                                       candidature_id AS candidatureId, composition_json AS compositionJson,
                                       created_at AS createdAt, updated_at AS updatedAt
                                  FROM working_cvs WHERE id = ?`).get(id) as unknown as WorkingRow | undefined;
...
function toLetter(row: LetterRow): CoverLetterRecord {
  return coverLetterRecordSchema.parse({
    ...
    bodyParagraphs: parseJson(row.bodyJson, (value) => coverLetterRecordSchema.shape.bodyParagraphs.parse(value), "Stored cover letter content is invalid."),
...
function readLetters(database: DatabaseSync): CoverLetterRecord[] {
  const rows = database.prepare(`SELECT id, candidature_id AS candidatureId, title, language,
                                        recipient, subject, body_json AS bodyJson, closing,
                                        created_at AS createdAt, updated_at AS updatedAt
                                   FROM cover_letters ORDER BY updated_at DESC, id DESC`)
...
function requireLetter(database: DatabaseSync, id: string): CoverLetterRecord {
  const row = database.prepare(`SELECT id, candidature_id AS candidatureId, title, language,
                                       recipient, subject, body_json AS bodyJson, closing,
                                       created_at AS createdAt, updated_at AS updatedAt
                                  FROM cover_letters WHERE id = ?`).get(id) as unknown as LetterRow | undefined;
  if (!row) throw new DocumentDomainServiceError("The cover letter no longer exists.");
  return toLetter(row);
}

function renderedProjectPath(rootPath: string, relativePath: string): string {
  return path.join(rootPath, relativePath);
}
function toRendered(row: RenderedRow): RenderedCvRecord {
  const snapshot = parseJson(row.snapshotJson, (value) => renderedCvSnapshotSchema.parse(value), "Stored Rendered CV snapshot is invalid.");
...
function readRenderedCvs(database: DatabaseSync): RenderedCvRecord[] {
  const rows = database.prepare(`SELECT id, working_cv_id AS workingCvId, source_template_id AS sourceTemplateId,
                                        candidature_id AS candidatureId, title, language,
                                        snapshot_json AS snapshotJson, project_relative_path AS projectRelativePath,
                                        created_at AS createdAt
                                   FROM rendered_cvs ORDER BY created_at DESC, id DESC`)
...
function requireRenderedRow(database: DatabaseSync, id: string): RenderedRow {
  const row = database.prepare(`SELECT id, working_cv_id AS workingCvId, source_template_id AS sourceTemplateId,
                                       candidature_id AS candidatureId, title, language,
                                       snapshot_json AS snapshotJson, project_relative_path AS projectRelativePath,
                                       created_at AS createdAt
                                  FROM rendered_cvs WHERE id = ?`).get(id) as unknown as RenderedRow | undefined;
  if (!row) throw new DocumentDomainServiceError("The Rendered CV no longer exists.");
  return row;
}

function toPacket(row: PacketRow): ApplicationPacketRecord {
...
function readPackets(database: DatabaseSync): ApplicationPacketRecord[] {
  const rows = database.prepare(`SELECT id, candidature_id AS candidatureId, rendered_cv_id AS renderedCvId,
                                        cover_letter_id AS coverLetterId, title,
                                        project_relative_path AS projectRelativePath, created_at AS createdAt
                                   FROM application_packets ORDER BY created_at DESC, id DESC`)
    .all() as unknown as PacketRow[];
  return rows.map(toPacket);
}
function requirePacketRow(database: DatabaseSync, id: string): PacketRow {
  const row = database.prepare(`SELECT id, candidature_id AS candidatureId, rendered_cv_id AS renderedCvId,
                                       cover_letter_id AS coverLetterId, title,
                                       project_relative_path AS projectRelativePath, created_at AS createdAt
                                  FROM application_packets WHERE id = ?`).get(id) as unknown as PacketRow | undefined;
...
function validateTemplateItems(rootPath: string, sections: readonly CvTemplateSection[]): void {
  ...
  for (const section of sections) {
    for (const item of section.items) {
      ...
      if (!base) throw new DocumentDomainServiceError("A CV template references My information that no longer exists.");
      if (item.sourceMode === "variant") {
        const variant = variants.get(item.profileVariantId);
        if (!variant || variant.itemId !== base.id) {
          throw new DocumentDomainServiceError("A CV template variant must belong to the selected My information item.");
        }
      }
    }
  }
...
function resolveTemplateItem(rootPath: string, item: CvTemplateItem): WorkingCvItem {
  ...
  if (item.sourceMode === "variant") {
    ...
    if (variant.itemId !== profileItem.id) {
      throw new DocumentDomainServiceError("The selected saved variant belongs to another My information item.");
    }
    return {
      id: randomUUID(),
      templateItemId: item.id,
...
function sectionsFromProfile(items: readonly ProfileItem[]): WorkingCvSection[] {
  const order = ["Profile", "Experience", "Projects / Selected work", "Education", "Skills", "Languages", "Links", "Other"];
  ...
  return order.flatMap((name) => {
    ...
    return groupedItems && groupedItems.length > 0 ? [{ id: randomUUID(), name, items: groupedItems }] : [];
  });
}
...
export function listDocumentCollections(rootPath: string): DocumentCollections {
  return withWorkspaceDatabase(rootPath, (database) => documentCollectionsSchema.parse({
    ...
    renderedCvs: readRenderedCvs(database),
    letters: readLetters(database),
    applicationPackets: readPackets(database),
  }));
}
...
export function createCvTemplate(rootPath: string, rawInput: CvTemplateInput): DocumentCollections {
  ...
  return withWorkspaceDatabase(rootPath, (database) => {
    ...
    transact(database, () => {
      database.prepare(`INSERT INTO cv_templates(id, name, language, composition_json, created_at, updated_at)
                        VALUES (?, ?, ?, ?, ?, ?)`)
        .run(id, input.name, nullable(input.language), JSON.stringify(input.sections), now, now);
    });
    return listDocumentCollections(rootPath);
  });
}

export function updateCvTemplate(rootPath: string, rawInput: CvTemplateUpdate): DocumentCollections {
  ...
  return withWorkspaceDatabase(rootPath, (database) => {
    ...
    transact(database, () => {
      ...
      database.prepare(`UPDATE cv_templates
                           SET name = ?, language = ?, composition_json = ?, updated_at = ?
                         WHERE id = ?`)
        .run(input.name, nullable(input.language), JSON.stringify(input.sections), now, input.id);
    });
    return listDocumentCollections(rootPath);
  });
}
...
export function createWorkingCv(rootPath: string, rawInput: WorkingCvCreate): WorkingCvRecord {
  ...
  if (input.source.kind === "profile") {
    ...
    const template = withWorkspaceDatabase(rootPath, (database) => requireTemplate(database, input.source.templateId));
  ...
  return withWorkspaceDatabase(rootPath, (database) => {
    ...
    const id = randomUUID();
    const now = new Date().toISOString();
    database.prepare(`INSERT INTO working_cvs(
      id, title, language, source_template_id, candidature_id, composition_json, created_at, updated_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
      .run(id, input.title, nullable(language), sourceTemplateId, input.candidatureId, JSON.stringify(sections), now, now);
...
export function updateWorkingCv(rootPath: string, rawInput: WorkingCvUpdate): WorkingCvRecord {
  ...
  return withWorkspaceDatabase(rootPath, (database) => {
    ...
    database.prepare(`UPDATE working_cvs
                         SET title = ?, language = ?, composition_json = ?, updated_at = ?
                       WHERE id = ?`)
      .run(input.title, nullable(input.language), JSON.stringify(input.sections), new Date().toISOString(), input.id);
...
export function saveWorkingCvItem(rootPath: string, rawInput: WorkingCvSaveItem): WorkingCvRecord {
  ...
  const working = withWorkspaceDatabase(rootPath, (database) => requireWorkingCv(database, input.workingCvId));
  const item = findWorkingItem(working, input.itemId);

  if (input.target === "profile") {
    if (!item.profileItemId) throw new DocumentDomainServiceError("Custom CV content has no My information item to update.");
    const base = getProfileItem(rootPath, item.profileItemId);
    updateProfileItem(rootPath, { id: base.id, item: { kind: base.kind, ...profileContent(item.content) } });
  } else if (input.target === "profile_variant") {
    if (!item.profileItemId) throw new DocumentDomainServiceError("Custom CV content cannot become a profile variant without a My information item.");
    if (!input.variantName) throw new DocumentDomainServiceError("Name the new profile variant.");
    createProfileVariant(rootPath, { itemId: item.profileItemId, name: input.variantName, content: profileContent(item.content) });
  } else {
    ...
    const template = withWorkspaceDatabase(rootPath, (database) => requireTemplate(database, working.sourceTemplateId ?? ""));
    const sections = template.sections.map((section) => ({
      ...section,
      items: section.items.map((candidate): CvTemplateItem => {
        ...
        return { id: candidate.id, sourceMode: "override", profileItemId: item.profileItemId, content: item.content };
      }),
    }));
    updateCvTemplate(rootPath, { id: template.id, name: template.name, language: template.language, sections });
  }
...
function templateSectionsFromWorking(working: WorkingCvRecord): CvTemplateSection[] {
  return working.sections.map((section) => ({
    id: randomUUID(),
    name: section.name,
    items: section.items.map((item): CvTemplateItem => item.profileItemId
      ? { id: randomUUID(), sourceMode: "override", profileItemId: item.profileItemId, content: item.content }
      : { id: randomUUID(), sourceMode: "custom", content: item.content }),
  }));
}

export function saveWorkingCvAsTemplate(rootPath: string, rawInput: WorkingCvSaveTemplate): CvTemplateRecord {
  const input = workingCvSaveTemplateSchema.parse(rawInput);
  const working = withWorkspaceDatabase(rootPath, (database) => requireWorkingCv(database, input.workingCvId));
  createCvTemplate(rootPath, { name: input.name, language: working.language, sections: templateSectionsFromWorking(working) });
  return withWorkspaceDatabase(rootPath, (database) => {
    const created = readTemplates(database).find((template) => template.name.toLocaleLowerCase() === input.name.toLocaleLowerCase());
...
function escapeLatex(value: string): string {
  return value.replace(/[\\{}$&#%_^~]/g, (character) => latexEscapes[character] ?? character).replace(/\r?\n/g, " \\\\ ");
}
function cvData(working: WorkingCvRecord): string {
  ...
  for (const section of working.sections) {
    ...
    for (const item of section.items) {
      ...
      lines.push(`\\AAAATEntry{${escapeLatex(item.content.kind)}}{${escapeLatex(item.content.title)}}{${escapeLatex(details)}}{${escapeLatex(item.content.description ?? "")}}`);
    }
  }
  return `${lines.join("\n")}\n`;
}
function coverLetterData(letter: CoverLetterRecord): string {
  ...
  for (const paragraph of letter.bodyParagraphs) lines.push(`\\AAAATParagraph{${escapeLatex(paragraph)}}`);
...
async function compileProject(projectPath: string, timeoutMs: number): Promise<void> {
  try {
    ...
    if (error instanceof LatexRunnerError) {
      throw new DocumentDomainServiceError(`${error.message} Check that latexmk and pdflatex are installed and compatible.`);
    }
    throw error;
  }
...
export async function renderWorkingCv(rootPath: string, workingCvId: string, timeoutMs = 30_000): Promise<RenderedCvRecord> {
  const working = withWorkspaceDatabase(rootPath, (database) => requireWorkingCv(database, workingCvId));
  const id = randomUUID();
  const relativePath = path.join("rendered-cvs", id);
  const projectPath = renderedProjectPath(rootPath, relativePath);
  ...
  try {
    ...
    withWorkspaceDatabase(rootPath, (database) => {
      database.prepare(`INSERT INTO rendered_cvs(
        ...
        .run(id, working.id, working.sourceTemplateId, working.candidatureId, working.title, nullable(working.language), JSON.stringify(snapshot), relativePath, createdAt);
    });
    return withWorkspaceDatabase(rootPath, (database) => toRendered(requireRenderedRow(database, id)));
  } catch (error) {
...
export function duplicateRenderedCv(rootPath: string, renderedCvId: string): WorkingCvRecord {
  const row = withWorkspaceDatabase(rootPath, (database) => requireRenderedRow(database, renderedCvId));
  const snapshot = parseJson(row.snapshotJson, (value) => renderedCvSnapshotSchema.parse(value), "Stored Rendered CV snapshot is invalid.");
  return withWorkspaceDatabase(rootPath, (database) => {
    const id = randomUUID();
    const now = new Date().toISOString();
    database.prepare(`INSERT INTO working_cvs(
      ...
      .run(id, `Copy of ${snapshot.title}`, nullable(snapshot.language), snapshot.sourceTemplateId, snapshot.candidatureId, JSON.stringify(snapshot.sections), now, now);
    return requireWorkingCv(database, id);
  });
}
...
export function renderedCvPdfPath(rootPath: string, renderedCvId: string): string {
  return withWorkspaceDatabase(rootPath, (database) => {
    ...
    const pdf = path.join(renderedProjectPath(rootPath, row.projectRelativePath), "build", "main.pdf");
    if (!existsSync(pdf)) throw new DocumentDomainServiceError("The retained Rendered CV PDF is missing.");
    return pdf;
  });
}

function safeProjectName(title: string, id: string): string {
  const slug = title.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 48) || "cv";
  return `${slug}-${id.slice(0, 8)}`;
}
export function exportRenderedCvProject(rootPath: string, renderedCvId: string, targetParent: string): string {
  const row = withWorkspaceDatabase(rootPath, (database) => requireRenderedRow(database, renderedCvId));
  try {
  ...
  const source = renderedProjectPath(rootPath, row.projectRelativePath);
  const destination = path.join(targetParent, safeProjectName(row.title, row.id));
  if (existsSync(destination)) throw new DocumentDomainServiceError("A portable project with that name already exists.");
...
export function createCoverLetter(rootPath: string, rawInput: CoverLetterInput): CoverLetterRecord {
  ...
  return withWorkspaceDatabase(rootPath, (database) => {
    ...
    const id = randomUUID();
    const now = new Date().toISOString();
    database.prepare(`INSERT INTO cover_letters(
      id, candidature_id, title, language, recipient, subject, body_json, closing, created_at, updated_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .run(id, input.candidatureId, input.title, nullable(input.language), nullable(input.recipient), nullable(input.subject), JSON.stringify(input.bodyParagraphs), nullable(input.closing), now, now);
    return requireLetter(database, id);
  });
}

export function updateCoverLetter(rootPath: string, rawInput: CoverLetterUpdate): CoverLetterRecord {
  const input = coverLetterUpdateSchema.parse(rawInput);
  return withWorkspaceDatabase(rootPath, (database) => {
    requireLetter(database, input.id);
    database.prepare(`UPDATE cover_letters
                         SET title = ?, language = ?, recipient = ?, subject = ?, body_json = ?, closing = ?, updated_at = ?
                       WHERE id = ?`)
      .run(input.title, nullable(input.language), nullable(input.recipient), nullable(input.subject), JSON.stringify(input.bodyParagraphs), nullable(input.closing), new Date().toISOString(), input.id);
...
export function removeCoverLetter(rootPath: string, letterId: string): DocumentCollections {
  return withWorkspaceDatabase(rootPath, (database) => {
    ...
    const packet = database.prepare("SELECT 1 FROM application_packets WHERE cover_letter_id = ? LIMIT 1").get(letterId);
    if (packet) throw new DocumentDomainServiceError("This cover letter belongs to a retained Application packet and cannot be removed.");
...
\newcommand{\AAAATIncludePDFPage}[1]{%
  \vbox to \textheight{%
    \vfil
    \hbox to \textwidth{\hfil\includegraphics[page=\the\AAAATPage,width=\textwidth,height=\textheight,keepaspectratio]{#1}\hfil}%
    \vfil
  }%
...
export async function createApplicationPacket(rootPath: string, rawInput: ApplicationPacketCreate, timeoutMs = 30_000): Promise<ApplicationPacketRecord> {
  ...
  try {
    ...
    cpSync(renderedProjectPath(rootPath, cvRow.projectRelativePath), path.join(stagePath, "cv"), { recursive: true, errorOnExist: true });
    ...
    withWorkspaceDatabase(rootPath, (database) => {
      database.prepare(`INSERT INTO application_packets(
        id, candidature_id, rendered_cv_id, cover_letter_id, title, letter_snapshot_json, project_relative_path, created_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
        .run(id, input.candidatureId, input.renderedCvId, input.coverLetterId, title, JSON.stringify(letter), relativePath, createdAt);
    });
    return withWorkspaceDatabase(rootPath, (database) => toPacket(requirePacketRow(database, id)));
  } catch (error) {
...
export function applicationPacketPdfPath(rootPath: string, packetId: string): string {
  return withWorkspaceDatabase(rootPath, (database) => {
    ...
    const pdf = path.join(rootPath, row.projectRelativePath, "build", "main.pdf");
    if (!existsSync(pdf)) throw new DocumentDomainServiceError("The retained Application packet PDF is missing.");
    return pdf;
  });
}
