...
const templateCurrentItemSchema = z
  .object({
    id: z.string().uuid(),
    sourceMode: z.literal("current"),
    profileItemId: z.string().uuid(),
  })
  .strict();
const templateVariantItemSchema = z
  .object({
    id: z.string().uuid(),
    sourceMode: z.literal("variant"),
    profileItemId: z.string().uuid(),
    profileVariantId: z.string().uuid(),
  })
  .strict();
const templateOverrideItemSchema = z
  .object({
    id: z.string().uuid(),
    sourceMode: z.literal("override"),
    profileItemId: z.string().uuid(),
    content: cvContentSchema,
  })
  .strict();
const templateCustomItemSchema = z
  .object({
    id: z.string().uuid(),
    sourceMode: z.literal("custom"),
    content: cvContentSchema,
  })
  .strict();
...
export const cvTemplateSectionSchema = z
  .object({
    id: z.string().uuid(),
...
export const cvTemplateInputSchema = z
  .object({
...
export const cvTemplateRecordSchema = cvTemplateInputSchema
  .extend({
    id: z.string().uuid(),
...
export const cvTemplateUpdateSchema = cvTemplateInputSchema
  .extend({ id: z.string().uuid() })
  .strict();
export type CvTemplateUpdate = z.infer<typeof cvTemplateUpdateSchema>;

export const workingCvItemSchema = z
  .object({
    id: z.string().uuid(),
  ...
  .superRefine((item, context) => {
    if (item.sourceMode === "custom" && (item.profileItemId !== null || item.profileVariantId !== null)) {
      context.addIssue({ code: "custom", message: "Custom CV content cannot point to My information." });
    }
    if (item.sourceMode !== "custom" && item.profileItemId === null) {
      context.addIssue({ code: "custom", message: "Profile-backed CV content needs a My information item." });
    }
    if (item.sourceMode === "variant" && item.profileVariantId === null) {
      context.addIssue({ code: "custom", message: "Variant CV content needs a saved item variant." });
    }
  });
export type WorkingCvItem = z.infer<typeof workingCvItemSchema>;

export const workingCvSectionSchema = z
  .object({
    id: z.string().uuid(),
    name: z.string().trim().min(1).max(120),
...
export const workingCvCreateSchema = z
  .object({
...
export const workingCvRecordSchema = z
  .object({
    id: z.string().uuid(),
    title: z.string().trim().min(1).max(200),
    language: optionalLanguageSchema,
...
export const workingCvUpdateSchema = workingCvRecordSchema
  .pick({ id: true, title: true, language: true, sections: true })
  .strict();
...
export const workingCvSaveItemSchema = z
  .object({
    workingCvId: z.string().uuid(),
    itemId: z.string().uuid(),
    target: workingCvOwnershipTargetSchema,
    variantName: z.string().trim().min(1).max(120).optional(),
  })
  .strict();
export type WorkingCvSaveItem = z.infer<typeof workingCvSaveItemSchema>;

export const workingCvSaveTemplateSchema = z
  .object({ workingCvId: z.string().uuid(), name: z.string().trim().min(1).max(200) })
  .strict();
...
export const renderedCvRecordSchema = z
  .object({
    id: z.string().uuid(),
    workingCvId: optionalUuidSchema,
    sourceTemplateId: optionalUuidSchema,
    candidatureId: optionalUuidSchema,
    title: z.string().min(1),
    language: optionalLanguageSchema,
    snapshot: renderedCvSnapshotSchema,
    createdAt: z.string().min(1),
    hasPdf: z.literal(true),
  })
  .strict();
...
export const coverLetterUpdateSchema = coverLetterInputSchema
  .omit({ candidatureId: true })
  .extend({ id: z.string().uuid() })
  .strict();
export type CoverLetterUpdate = z.infer<typeof coverLetterUpdateSchema>;

export const applicationPacketCreateSchema = z
  .object({
    candidatureId: z.string().uuid(),
    renderedCvId: z.string().uuid(),
...
export const applicationPacketRecordSchema = z
  .object({
    id: z.string().uuid(),
    candidatureId: z.string().uuid(),
    renderedCvId: z.string().uuid(),
...
export const documentCollectionsSchema = z
  .object({
...
export const portableProjectExportResultSchema = z
