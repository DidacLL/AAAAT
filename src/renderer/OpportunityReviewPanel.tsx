...
export function OpportunityReviewPanel({ record }: Props) {
  ...
  const request = { candidatureId: record.id } as const;
  ...
  return (
    <section className="focus-concepts" aria-label="AI opportunity review">
      <div>
        ...
        <p>
          AAAAT builds a read-only context from saved candidature and professional information
          whose AI-use eye is enabled. Unsaved candidature edits are not included. The operation
      ...
      {preview ? (
        <section className="selected-concept-definition">
          ...
          <p>
            ...
            {isLocalConnection(preview.connection.endpoint)
      ...
      {result ? (
        <article className="selected-concept-definition" aria-label="Opportunity review result">
          ...
          <ul>
            {result.relevantEvidence.map((item) => (
              <li key={item}>{item}</li>
            ))}
          </ul>
          <h4>Uncertainties</h4>
