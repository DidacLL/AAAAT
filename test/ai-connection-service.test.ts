...
function provider(): ModelProvider {
  return {
    ...
    tailorCv: vi.fn<ModelProvider["tailorCv"]>(async () => ({
      recommendations: [{ itemRef: "aaaat_validation_item", rationale: "Synthetic validation result" }],
    })),
...
describe("named AI connections", () => {
  it("routes operations only through explicitly validated capabilities", async () => {
    ...
    const firstSave = saveNamedAiConnection(root, { name: "Fast local", endpoint: "http://localhost:11434/v1", model: "fast-model" });
    ...
    const firstValidated = await validateAiConnectionOperation(root, { connectionId: first.id, operation: "opportunity_review" }, provider());
    expect(firstValidated.find((connection) => connection.id === first.id)).toMatchObject({ validatedOperations: ["opportunity_review"], defaultForOperations: ["opportunity_review"] });
    expect(getAiConnectionForOperation(root, "opportunity_review")?.name).toBe("Fast local");

    const bothValidated = await validateAiConnectionOperation(root, { connectionId: second.id, operation: "opportunity_review" }, provider());
    expect(bothValidated.find((connection) => connection.id === first.id)?.defaultForOperations).toEqual(["opportunity_review"]);
    expect(bothValidated.find((connection) => connection.id === second.id)?.defaultForOperations).toEqual([]);

    const switched = setAiOperationDefault(root, { connectionId: second.id, operation: "opportunity_review" });
    expect(switched.find((connection) => connection.id === second.id)?.defaultForOperations).toEqual(["opportunity_review"]);
    expect(getAiConnectionForOperation(root, "opportunity_review")?.name).toBe("Deep local");

    const renamed = saveNamedAiConnection(root, { id: second.id, name: "Deep local renamed", endpoint: second.endpoint, model: second.model });
    expect(renamed.find((connection) => connection.id === second.id)).toMatchObject({ validatedOperations: ["opportunity_review"], defaultForOperations: ["opportunity_review"] });
  ...
  it("does not overwrite a connection changed while capability validation is running", async () => {
    ...
    blockingProvider.reviewOpportunity = vi.fn<ModelProvider["reviewOpportunity"]>(async () => {
      markStarted?.(); await validationGate;
      return { summary: "Synthetic validation result", relevantEvidence: [], uncertainties: [], questions: [] };
    });

    const pending = validateAiConnectionOperation(root, { connectionId: connection.id, operation: "opportunity_review" }, blockingProvider);
    await started;
    saveNamedAiConnection(root, { id: connection.id, name: connection.name, endpoint: connection.endpoint, model: "model-b" });
    releaseValidation?.();
  ...
  it("rejects invalid routing, ambiguous names, unsafe endpoints, and obsolete development config", async () => {
    ...
    expect(() => setAiOperationDefault(root, { connectionId: connection.id, operation: "cv_tailoring" })).toThrow("is not validated for CV tailoring");

    const failingProvider = provider();
    failingProvider.reviewOpportunity = vi.fn<ModelProvider["reviewOpportunity"]>(async () => { throw new Error("synthetic validation failed"); });
    await expect(validateAiConnectionOperation(root, { connectionId: connection.id, operation: "opportunity_review" }, failingProvider)).rejects.toThrow("synthetic validation failed");
