...
class AssistancePanel(wx.ScrolledWindow):
    """Plain-language AI status with advanced local fallbacks."""
    ...
    def _build_connection_section(self) -> None:
        ...
        intro = wx.StaticText(
            self,
            label=(
                "Connect the AI you already use. It can choose the best connection "
                "it supports and help with selected preparation work, while AAAAT "
                "keeps your workspace local."
            ),
        )
        intro.Wrap(760)
        self.root.Add(
        ...
        current = dict(
            self.snapshot.get("connection")
            or self.snapshot.get("integration")
            or {}
        )
        ...
        disclosure = wx.StaticText(
            ...
            label=(
                ...
                "Your AI account, credentials, and data policy remain under your "
                "control."
            ),
        )
        disclosure.Wrap(760)
        ...
        if state in {"connected", "needs_attention"}:
            ...
            connect.Bind(
                wx.EVT_BUTTON,
                lambda _event: self._select_mode("guided_connector"),
            )
        ...
        advanced.Bind(
            wx.EVT_BUTTON,
            lambda _event: self._select_mode("advanced_integration"),
        )
    ...
    def _connection_status(connection: dict[str, Any]) -> str:
        state = str(
            connection.get("state") or connection.get("status") or ""
        ).strip().lower()
        if state == "connected":
            return "Connected"
        if state == "paused":
    ...
    def _disconnect(self, _event: wx.CommandEvent) -> None:
        try:
            ...
            return
        self.integration_status.SetLabel("AI connection paused")
    ...
    def _select_mode(self, mode_id: str) -> None:
        ...
        try:
            ...
            return
        if result:
            self.integration_status.SetLabel(
                str(result.get("message") or result.get("status") or "Ready")
            )
    ...
    def _build_advanced_section(self, current: dict[str, Any]) -> None:
        ...
        helper = wx.StaticText(
            self,
            label=(
                "Configure a controlled file exchange or a user-owned command. "
                "AAAAT passes only one bounded task and accepts one validated result. "
                "Saving checks only the local settings and primitive availability; "
                "AAAAT does not certify the external host."
            ),
        )
        helper.Wrap(760)
        self.root.Add(
        ...
        readiness = dict(self.snapshot.get("readiness") or {})
        readiness_label = wx.StaticText(
            self,
            label=(
                f"Local readiness: {readiness.get('status') or 'unknown'} · "
                f"{readiness.get('message') or 'No advanced route selected.'}"
            ),
        )
        readiness_label.Wrap(760)
        self.root.Add(
            readiness_label,
            0,
            wx.LEFT | wx.RIGHT | wx.BOTTOM | wx.EXPAND,
        ...
        options = [
            item
            for item in list(self.snapshot.get("options") or [])
            if bool(item.get("advanced"))
        ]
        self.option_by_title = {
            str(item.get("title") or item.get("id")): item
            for item in options
        }
        ...
        self.choice.SetSelection(
            titles.index(selected_title)
            if selected_title in titles
            else (0 if titles else wx.NOT_FOUND)
        )
        ...
        save = wx.Button(self, label="Validate and save advanced integration")
    ...
    def _populate_fields(self) -> None:
        ...
        current = dict(
            (self.snapshot.get("integration") or {}).get("settings") or {}
        )
        if str(option.get("id") or "") != str(
            (self.snapshot.get("integration") or {}).get("id") or ""
        ):
            current = recommended
    ...
    def _settings(self) -> dict[str, Any]:
        ...
        fields = {
            ...
            for item in list(option.get("fields") or [])
        }
        values: dict[str, Any] = {}
        for key, control in self.field_controls.items():
            ...
            values[key] = (
                value.splitlines()
                if bool(fields.get(key, {}).get("multiline"))
                else value
            )
        return values
    ...
    def _save(self, _event: wx.CommandEvent) -> None:
        ...
        self.integration_status.SetLabel(
            str(health.get("message") or result.get("status") or "Saved")
        )
    ...
    def _build_task_section(self) -> None:
        ...
        if not tasks:
            self.root.Add(
                wx.StaticText(self, label="No assistance tasks yet."),
                0,
                wx.LEFT | wx.RIGHT | wx.BOTTOM,
                10,
            )
        ...
        for task in tasks:
            ...
            if task.get("can_run"):
                ...
                button.Bind(
                    wx.EVT_BUTTON,
                    lambda _event, value=task_id: self.on_run_task(value),
                )
            ...
            if task.get("can_retry"):
                ...
                button.Bind(
                    wx.EVT_BUTTON,
                    lambda _event, value=task_id: self.on_retry_task(value),
                )
                actions.Add(button, 0, wx.RIGHT, 6)
            if task.get("can_cancel"):
                ...
                button.Bind(
                    wx.EVT_BUTTON,
                    lambda _event, value=task_id: self.on_cancel_task(value),
                )
    ...
    def _create_profile_task(self, _event: wx.CommandEvent) -> None:
        try:
            ...
            self.integration_status.SetLabel(
                ...
                + str(
                    result.get("title")
                    or "Complete professional profile"
                )
            )
        except Exception as exc:
            self.integration_status.SetLabel(str(exc))
