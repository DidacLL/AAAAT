...
class UserViewMixin:
    ...
    def _build_user_surface(self) -> None:
        ...
        self.evidence_scroll = wx.ScrolledWindow(
            self.user_workspace,
            style=wx.VSCROLL,
        )
    ...
    def _render_profile_facts(self) -> None:
        ...
        self.evidence_sizer.Add(
            self.profile_facts_panel,
            0,
            wx.ALL | wx.EXPAND,
            10,
        )
    ...
    def _save_integration(
        ...
        self.SetStatusText(
            ...
            if result.get("saved")
            else "Advanced integration settings need attention"
        )
    ...
    def _select_connection_mode(self, mode_id: str) -> dict[str, Any]:
        if mode_id == "manual":
            ...
            self.SetStatusText(
                "Manual use selected; AI assistance remains optional"
            )
            self._schedule_assistance_refresh()
        ...
        if mode_id == "advanced_integration":
            ...
            return {
                "status": "ready",
                "message": "Advanced integration settings opened.",
            }
    ...
    def _ensure_task_worker(self) -> OwnedTaskWorker:
        if self._task_worker is None:
            self._task_worker = OwnedTaskWorker(
                self.storage_path,
                on_event=self._on_task_worker_event,
            )
    ...
    def _apply_task_worker_event(self, event: dict[str, Any]) -> None:
        ...
        if task_id:
            ...
            if int(event.get("sequence") or 0) >= int(
    ...
    def _export_portable_bundle(self) -> dict[str, Any] | None:
        ...
        if not candidature_ref:
            wx.MessageBox(
                "Select a candidature in Smart or Detailed view first.",
