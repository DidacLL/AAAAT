...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--debug",
        action="store_true",
        help="Show a traceback for an expected command failure.",
    )
    ...
    restore_p = sub.add_parser(
        "restore",
        help="Restore a backup into a new or empty workspace directory.",
    )
    ...
    agent.add_parser(
        "next",
        help=(
            "Atomically claim and return one bounded work item with its "
            "purpose-scoped context and response schema."
        ),
    )
    ...
    agent_action = agent.add_parser("action").add_subparsers(
    ...
    host_install_skill = host.add_parser(
        "install-skill",
        help="Install the AAAAT skill in a host-selected directory.",
    )
    host_install_skill.add_argument(
        "--directory",
        required=True,
        help="Host-owned skills directory selected under the host permission policy.",
    )
    ...
    intake = sub.add_parser("intake").add_subparsers(
        dest="intake_command",
        required=True,
    )
    ...
    render = sub.add_parser("render").add_subparsers(
        dest="render_command",
        required=True,
    )
    ...
    profile_fact = profile.add_parser("fact").add_subparsers(
        dest="fact_command",
        required=True,
    )
    ...
    profile_fact_add.add_argument(
        "--visibility",
        default="private",
        choices=["public", "professional", "private", "sensitive"],
    )
    profile_fact_add.add_argument(
        "--exposure",
        default="summarized",
    ...
    for field in ("type", "title", "body", "tags", "visibility", "exposure"):
        profile_fact_update.add_argument(
            f"--{field}",
            dest="fact_type" if field == "type" else field,
        )
    profile_fact_archive = profile_fact.add_parser("archive")
    profile_fact_archive.add_argument("id")

    career_plan = sub.add_parser("career-plan").add_subparsers(
        dest="career_plan_command",
        required=True,
    )
    ...
    glossary = sub.add_parser("glossary").add_subparsers(
    ...
    keyword = sub.add_parser("keyword").add_subparsers(
        dest="keyword_command",
        required=True,
    )
    ...
    task = sub.add_parser("task").add_subparsers(
        dest="task_command",
        required=True,
    )
    ...
    todo = sub.add_parser("todo").add_subparsers(
        dest="todo_command",
        required=True,
    )
    ...
    note = sub.add_parser("note").add_subparsers(
        dest="note_command",
        required=True,
    )
    ...
    blob = sub.add_parser("blob").add_subparsers(
        dest="blob_command",
        required=True,
    )
    ...
    variable = sub.add_parser("variable").add_subparsers(
        dest="variable_command",
        required=True,
    )
    ...
    variable_set.add_argument(
        "--exposure",
        default="placeholder",
        choices=["raw", "redacted", "summarized", "placeholder", "denied"],
    )
...
def _run(argv: list[str] | None = None) -> int:
    ...
    if args.command == "host" and args.host_command == "install-skill":
        target = Path(args.directory) / "AAAAT" / "SKILL.md"
    ...
    with connect(args.storage) as conn:
        if args.command == "agent" and args.agent_command == "next":
            ...
            args.command == "agent"
            and args.agent_command == "action"
            and args.agent_action_command == "submit"
        ):
            action_body = (
                Path(args.input_file).read_text(encoding="utf-8")
                if args.input_file
                else args.input_body
            )
            _json(
                submit_agent_action(
                    conn,
            ...
            _json(
                update_application(
                    conn,
                    args.id,
                    **_provided_fields(args, set(APPLICATION_FIELDS)),
                )
            )
            ...
            _json(
                render_document_artifact(
                    ...
                    args.output,
                    args.application_id,
                    {"artifact.cover_letter.body": args.body},
                    compile_pdf=args.compile_pdf,
                    save_version=True,
                )
            )
            ...
            and args.profile_command == "fact"
            and args.fact_command == "add"
        ):
            _json(
                create_profile_fact(
                    conn,
                    ...
                    use_for_desktop=not args.hide_from_desktop,
                    source="cli",
                )
            )
        elif (
            args.command == "profile"
            and args.profile_command == "fact"
            and args.fact_command == "list"
        ):
            _json(list_profile_facts(conn))
        elif (
            args.command == "profile"
            and args.profile_command == "fact"
            and args.fact_command == "show"
        ):
            _json(get_profile_fact(conn, args.id))
        elif (
            args.command == "profile"
            and args.profile_command == "fact"
            and args.fact_command == "update"
        ):
            fields = {
                key: value
                for key, value in vars(args).items()
                if key
                in {"fact_type", "title", "body", "tags", "visibility", "exposure"}
            ...
            fields = {
                key: value
                for key, value in vars(args).items()
                if key in {"state", "title", "body", "pinned"}
...
def _command_error_message(exc: BaseException) -> str:
    ...
    if isinstance(exc, TemplateVariableError):
        return (
            f"{message}. Complete the missing profile fields with "
            "'aaaat profile set <field> <value>', then retry."
        )
    return message


def main(argv: list[str] | None = None) -> int:
    """Run the local maintenance CLI without hiding programming failures."""
