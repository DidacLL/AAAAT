...
LIFECYCLE_TASKS: tuple[LifecycleTaskSpec, ...] = (
    LifecycleTaskSpec(
        "extract",
        "field_inference",
        "Extract and enrich candidature",
        "Extract only grounded missing candidature fields from retained source material. Preserve user-entered values.",
    ...
    LifecycleTaskSpec(
        "strategy",
        "field_inference",
        "Prepare role-specific strategy",
    ...
    LifecycleTaskSpec(
        ...
        "Prepare concise recruiter-call notes, likely questions, positioning, logistics and risks.",
        "call:recruiter",
        "normal",
...
def lifecycle_plan(
    ...
    *,
    ...
    for spec in LIFECYCLE_TASKS:
        ...
        if existing_task:
            item = _planned(spec, str(existing_task.get("state") or "queued"), str(existing_task.get("notes") or ""))
            item["task_id"] = str(existing_task.get("id") or "")
            plan.append(item)
            continue
        missing = [field for field in spec.requires if not str(current.get(field) or "").strip()]
        plan.append(_planned(spec, "blocked" if missing else "ready", "Missing prerequisites: " + ", ".join(missing) if missing else ""))
    return plan


def ensure_lifecycle_tasks(
    conn: sqlite3.Connection,
    candidature_ref: str,
    *,
    research_capable: bool = False,
