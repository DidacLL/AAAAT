...
_SAMPLE_OFFER = """Senior Backend Engineer — Developer Productivity\n\nNorthstar Tools builds developer infrastructure for local environments, release workflows, and internal automation. The role requires Python, SQLite, subprocess integration, deterministic testing, packaging, failure recovery, and pragmatic architecture. Remote within Spain or hybrid from Madrid. Salary range: €68,000–€82,000."""
_SAMPLE_FORM = """1. Why are you interested in Northstar Tools?\n2. Describe a system where you improved reliability.\n3. Describe your Python and SQLite experience.\n4. How do you avoid overengineering?"""
...
class ReleaseValidator:
    ...
    def run(self) -> dict[str, Any]:
        ...
        stages: list[tuple[str, Callable[[StageResult], None]]] = [
        ...
        for name, callback in stages:
            ...
            if self.results[-1].status == "failed":
                break
        automated = VERDICT_PASSED if all(item.status == "passed" for item in self.results) else VERDICT_FAILED
        report = {
    ...
    def _run_stage(self, name: str, callback: Callable[[StageResult], None]) -> None:
        ...
        (self.config.evidence_dir / f"stage-{name}.json").write_text(
    ...
    def _stage_environment(self, item: StageResult) -> None:
        ...
        path = self.config.evidence_dir / "environment.json"
    ...
    def _stage_empty_store(self, item: StageResult) -> None:
        ...
        with connect(self.config.storage) as conn:
            ...
            _require(profile["profile.display_name"] == "Alex Rivera", "Profile seed was not retained")
    ...
    def _stage_runtime_configuration(self, item: StageResult) -> None:
        ...
        path = self.config.evidence_dir / "runtime-configuration.json"
    ...
    def _stage_runtime_conformance(self, item: StageResult) -> None:
        ...
        path = self.config.evidence_dir / "runtime-conformance.json"
        path.write_text(json.dumps(result, ensure_ascii=False, indent=2, sort_keys=True) + "\n", encoding="utf-8")
        item.evidence.append(str(path))
    ...
    def _stage_profile_completion(self, item: StageResult) -> None:
        with connect(self.config.storage) as conn:
            conn.execute("DELETE FROM profile_variables WHERE key IN ('profile.career_direction', 'profile.availability')")
            conn.commit()
    ...
    def _stage_candidature_lifecycle(self, item: StageResult) -> None:
        ...
        with connect(self.config.storage) as conn:
            ...
            for key in ("candidature_evaluation", "role_strategy", "company_research", "form_answers", "cv_material", "cover_letter_material", "recruiter_material", "questions_to_ask"):
    ...
    def _stage_privacy_audit(self, item: StageResult) -> None:
        ...
        path = self.config.evidence_dir / "bounded-task-packet.json"
    ...
    def _stage_projection_visibility(self, item: StageResult) -> None:
        ...
        path = self.config.evidence_dir / "dashboard-payload.json"
    ...
    def _runtime_settings(self) -> tuple[str, dict[str, Any]]:
        if self.config.runtime == "deterministic":
            self._runtime_script = self.config.evidence_dir / "deterministic-runtime.py"
            self._runtime_script.write_text(_deterministic_runtime_source(), encoding="utf-8")
            return "argv_custom_command", {"argv": [sys.executable, str(self._runtime_script)], "timeout_seconds": self.config.timeout_seconds}
    ...
    def _write_reports(self, report: dict[str, Any]) -> None:
        json_path = self.config.evidence_dir / "release-report.json"
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(json.dumps({"automated_verdict": report["automated_verdict"], "release_verdict": report["release_verdict"], "report": str(config.evidence_dir / "release-report.json")}, indent=2))
