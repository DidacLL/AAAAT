...
def preparation_queue(payload: dict[str, Any], application_id: str | None = None) -> list[dict[str, Any]]:
    ...
    for app in payload.get("applications", []):
        if application_id and app.get("id") != application_id:
            continue
        if _needs_source_extraction(app):
        ...
        for field, action in PREPARATION_FIELDS.items():
...
def _needs_source_extraction(app: dict[str, Any]) -> bool:
    company = str(app.get("company") or "").strip().lower()
    role = str(app.get("role") or "").strip().lower()
    return not company or not role
...
def missing_preparation_count(app: dict[str, Any], glossary: dict[str, dict[str, Any]] | None = None) -> int:
    count = sum(1 for field in PREPARATION_FIELDS if not str(app.get(field) or "").strip())
...
def sorted_applications(applications: list[dict[str, Any]], glossary: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    def sort_key(app: dict[str, Any]) -> tuple[int, int, str, str]:
        ...
        missing = missing_preparation_count(app, glossary_map)
