        ...
        tasks = [
            {
                ...
                "can_cancel": advanced_command_active and str(task.get("state") or "") in {"queued", "claimed", "in_progress"},
                "progress": dict(persisted_progress.get(str(task.get("id") or "")) or {}),
            }
...
def _latest_progress_by_task(conn: Any) -> dict[str, dict[str, Any]]:
    table = conn.execute(
        "SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'agent_task_progress'"
    ).fetchone()
    if not table:
        return {}
    rows = conn.execute(
        """SELECT progress.task_id, progress.sequence, progress.phase, progress.message,
                  progress.percent, progress.created_at
           ...
           JOIN (
             ...
             ON latest.task_id = progress.task_id AND latest.sequence = progress.sequence"""
    ).fetchall()
    return {
        str(row["task_id"]): {
...
def create_profile_completion_task(storage_path: str | Path) -> dict[str, Any]:
    with connect(storage_path) as conn:
        return create_task(
            ...
            instructions="Discuss the profile naturally and return only eligible missing values the user chooses to provide. Preserve non-empty user values.",
            state="queued",
            priority="high",
