...
from .tasks import FIELD_INFERENCE_ALLOWED, complete_task, get_task, keyword_from_context
...
DEFAULT_TASK_INSTRUCTIONS = {
    "profile_completion": (
        "Collect eligible missing professional-profile fields through normal "
        "conversation. Existing non-empty desktop values remain authoritative."
    ),
    "field_inference": (
        "Complete only the lifecycle fields declared by this work item using its "
        "bounded candidature and profile context."
    ),
    "company_research": (
        "Prepare concise company research relevant to the candidature and role."
    ),
    "keyword_definition": (
        "Define the keyword for this job-search context when its canonical "
        "definition is empty."
    ),
    "draft_form_responses": (
        "Draft application form responses using the supplied form, application "
        "strategy and bounded profile context."
    ),
    "draft_cv": (
        "Suggest CV positioning and role-specific adaptation notes using the "
        "supplied fit and strategy. AAAAT renders final files locally."
    ),
    "draft_cover_letter": (
        "Draft a cover-letter body using the supplied fit, strategy and bounded "
        "profile context. AAAAT renders final files locally."
    ),
    "recruiter_call_material": (
        "Prepare concise recruiter-call material from the supplied candidature, "
        "fit, strategy and bounded profile context."
    ),
    "career_plan_review": (
        "Review the bounded career plan context and propose concrete improvements."
    ),
}
...
def task_capability(conn: sqlite3.Connection, task: dict[str, Any]) -> str:
    ...
    try:
        conn.execute(
            """INSERT INTO agent_task_capabilities(capability, task_id, created_at)
            VALUES (?, ?, ?)""",
...
def task_id_for_capability(conn: sqlite3.Connection, capability: str) -> str:
    ...
    row = conn.execute(
        "SELECT task_id FROM agent_task_capabilities WHERE capability = ?",
        (cleaned,),
    ).fetchone()
    if row is None:
        raise KeyError("Task capability not found")
...
def _field_task_config(task: dict[str, Any]) -> dict[str, Any]:
    ...
    if hint.startswith("field:"):
        ...
        if field not in FIELD_INFERENCE_ALLOWED:
            return {
                "purpose": "unsupported_field_request",
                "fields": set(),
                "profile_purpose": "",
            }
...
def task_instructions(task: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "default": DEFAULT_TASK_INSTRUCTIONS.get(
            task_type,
            "Complete the bounded AAAAT task using only the supplied context.",
        ),
...
def _writes_description(task: dict[str, Any]) -> str:
    ...
    return {
        ...
        "keyword_definition": (
            "Definition and optional category for a keyword whose canonical definition is empty."
        ),
    ...
    formats: dict[str, dict[str, Any]] = {
        ...
        "company_research": {
            ...
            "schema": {
                ...
                "sources_checked": {"type": "array", "optional": True},
            },
        },
        "keyword_definition": {
            "type": "json_object",
        ...
        "draft_form_responses": {
            ...
            "additional_properties": False,
            "schema": {
                "form_answers": {"type": ["string", "object"]},
                "assumptions": {"type": "string", "optional": True},
            },
        },
        "draft_cv": {
            "type": "json_object",
            ...
            "additional_properties": False,
            "schema": {
                "cv_positioning": {"type": "string"},
                "adaptation_notes": {"type": "string", "optional": True},
            },
        },
        "draft_cover_letter": {
            "type": "json_object",
...
def task_privacy_notes() -> list[str]:
    return [
        ...
        "task_capability is a random task-scoped callback capability, not a database or entity ID",
...
def _task_context(
    ...
    if application_id:
        ...
        source = "\n\n".join(
            ...
            for item in list_raw_intake(conn, application_id)
        )
        ...
        if task_type == "field_inference":
            ...
            context: dict[str, Any] = {
                ...
                "current_fields": {
                    field: current.get(field, "") for field in allowed_fields
                },
                "missing_fields": [
                    field
                ...
                "candidature_context": {
                    ...
                    "candidature_evaluation": details.get(
                        "candidature_evaluation",
                        "",
                    ),
            ...
            if profile_purpose:
                context["profile_context"] = _agent_profile_context(
                    conn,
                    profile_purpose,
                )
            return context
    ...
    if task_type == "career_plan_review":
        return {
            "career_plan": career_plan_context(
                conn,
...
def submit_agent_task_result(
    ...
    *,
    ...
    if "submit_result" not in allowed_actions(task):
        raise ValueError(
            f"Task is not accepting results in state {task.get('state')}"
        )
...
def _without_agent_control_keys(result_body: str) -> str:
    if len(result_body.encode("utf-8", errors="replace")) > _MAX_RESULT_BYTES:
        raise ValueError("Result exceeds the 1 MB safety limit")
    try:
    ...
    stripped = _strip_agent_control_keys(value)
    _validate_bounded_json(stripped)
    return json.dumps(stripped, ensure_ascii=False)


def _with_trusted_desktop_refresh(
    task: dict[str, Any],
    result_body: str,
) -> str:
    hint = safe_context_hint(task.get("context_hint"))
    if (
        str(task.get("created_by") or "") != "desktop_action"
        or not hint.startswith("field:")
    ):
        return result_body
    value = json.loads(result_body)
...
def _validate_bounded_json(value: Any, *, depth: int = 0) -> None:
    ...
    if isinstance(value, str):
        if len(value) > _MAX_STRING_CHARS:
            raise ValueError("Result contains an oversized text value")
        return
    if isinstance(value, dict):
        if len(value) > _MAX_CONTAINER_ITEMS:
            raise ValueError("Result object contains too many properties")
        for key, item in value.items():
            if len(str(key)) > _MAX_KEYWORD_CHARS:
                raise ValueError("Result object contains an oversized property name")
            _validate_bounded_json(item, depth=depth + 1)
        return
    if isinstance(value, list):
        if len(value) > _MAX_CONTAINER_ITEMS:
            raise ValueError("Result array contains too many items")
        for item in value:
            _validate_bounded_json(item, depth=depth + 1)
        return
...
def _validate_result_shape(task: dict[str, Any], result_body: str) -> None:
    ...
    if missing:
        raise ValueError(
            "Result does not match this task's required fields: "
            + ", ".join(missing)
        )
    ...
    if task_type == "field_inference":
        ...
        if unsupported:
            raise ValueError(
                "Field result contains unsupported candidature fields: "
                + ", ".join(unsupported)
            )
        for key, item in fields.items():
            if key == "keywords":
                ...
                if len(item) > _MAX_KEYWORDS:
                    raise ValueError("keywords contains too many values")
                if any(
                    not isinstance(term, str)
                    or not term.strip()
                    or len(term) > _MAX_KEYWORD_CHARS
                    for term in item
                ):
                    raise ValueError("keywords must contain bounded non-empty text values")
            elif not isinstance(item, (str, int, float, bool)):
                raise ValueError(
                    f"Candidature field must be a bounded scalar value: {key}"
                )
        return
