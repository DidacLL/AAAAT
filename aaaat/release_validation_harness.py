...
class ReleaseValidator:
    ...
    def _stage_environment(self, item: StageResult) -> None:
        ...
        path.write_text(json.dumps({"python": sys.version, "platform": platform.platform(), "runtime_profile": self.config.runtime}, indent=2) + "\n", encoding="utf-8")
    ...
    def _write_reports(self, report: dict[str, Any]) -> None:
        (self.config.evidence_dir / "release-report.json").write_text(json.dumps(report, ensure_ascii=False, indent=2, sort_keys=True) + "\n", encoding="utf-8")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command_json:
        ...
        command = value
    report = ReleaseValidator(ValidationConfig(storage=args.storage, evidence_dir=args.evidence_dir, runtime=args.runtime, model=args.model, model_path=args.model_path, executable=args.executable, args=list(args.args), command=command, timeout_seconds=args.timeout_seconds, keep_storage=args.keep_storage)).run()
