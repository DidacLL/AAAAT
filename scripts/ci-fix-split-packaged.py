...
# Restore the established chooser helper for the long-running security journey,
# while retaining the demo-specific chooser for the new focused acceptance test.
anchor = "function chooseLinuxDemoDirectory(): void {\n"
if anchor not in text:
    raise RuntimeError("demo chooser anchor missing")
choose_workspace = r'''function chooseLinuxDirectory(): void {
  execFileSync(
    "bash",
    [
      "-lc",
...
'''
text = text.replace(anchor, choose_workspace + anchor, 1)
...
start = text.find(start_marker, text.find('test("packaged desktop preserves security gates'))
end = text.find(end_marker, start)
if start < 0 or end < 0:
    raise RuntimeError("security journey demo block anchors missing")
restore = '''    await running.page.getByRole("button", { name: "Create workspace" }).click();
...
# Keep the new first-run affordance assertion and bounded API checks in the
# security journey, but exercise the heavier product path independently.
insert_before = 'test("packaged desktop preserves security gates and required bounded capabilities", async () => {\n'
if insert_before not in text:
    raise RuntimeError("security test anchor missing")
focused = r'''test("packaged demo workspace exposes offer, prompt guidance, and reset", async () => {
  ...
  try {
    ...
    const advanced = running.page
      .locator("summary")
      .filter({ hasText: "Advanced: AI instructions and context" });
    await advanced.click();
    const coverPrompt = running.page
      .locator(".document-card")
      .filter({ hasText: "Cover letter draft" })
      .first();
    await coverPrompt
      .getByLabel("Optional guidance")
      .fill("Prefer short paragraphs in packaged verification.");
...
'''
text = text.replace(insert_before, focused + insert_before, 1)
