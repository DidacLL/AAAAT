...
3. **Local/light-model usefulness is a PLAN[0] product requirement.** AAAAT's direct-AI baseline is modest local hardware running a small model through llama.cpp/OpenAI-compatible HTTP. Premium hosted models are not the correctness or UX baseline. Provider requests must minimize context and decoding burden, avoid unsupported/needlessly strict JSON-Schema features, keep deterministic validation in AAAAT, and salvage valid partial results independently. Tests and acceptance evidence must exercise imperfect small-model behavior rather than assuming premium-tier instruction following or schema fidelity.
