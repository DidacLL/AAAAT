...
3. **Local/light models are the primary AI target, not a fallback.** PLAN[0] AI extraction must be designed and tested first against modest local OpenAI-compatible models (llama.cpp-class runtime, small Qwen-class model), with premium/hosted models treated only as optional upside. Remove provider-facing schema features unsupported by that path, including the current date regex `pattern`; keep exact date/type validation locally. Audit and reduce request/schema/context burden where it assumes a stronger model: field-by-field schema expansion, oversized instructions/context, unnecessary strictness, and all-or-nothing parsing. Valid partial proposals survive independently when other proposals fail. Focused verification must include an imperfect small-local-model loop, not merely mocks that behave like a premium structured-output model.
