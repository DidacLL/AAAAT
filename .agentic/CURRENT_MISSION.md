...
3. **Affordable light-model use is the primary AI target, without provider lock-in.** PLAN[0] AI extraction must be designed for resource-constrained models first, including local models on ordinary hardware, while keeping product semantics provider/runtime/model-family agnostic. The current OpenAI-compatible adapter and llama.cpp/small-Qwen test fixtures are implementation/testing examples only. Remove provider-facing constraints that add model burden or fail on supported lightweight paths; keep deterministic date/type/domain validation locally. Reduce field-by-field schema expansion, oversized instructions/context, unnecessary strictness and all-or-nothing parsing. Valid partial proposals survive independently. Verification must exercise at least one imperfect resource-constrained model path without turning that fixture into product authority.
