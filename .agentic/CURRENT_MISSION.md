...
3. **Small OpenAI-compatible models must remain viable.** The job-extraction structured-output schema must avoid provider-facing JSON-Schema features known to be unsupported by the target llama.cpp path (including the current date regex `pattern`). Keep exact date/type validation locally after model output. Prefer a compact provider-facing contract plus local validation/recovery over a large field-by-field schema that degrades small-model reliability. Preserve valid partial proposals when other proposals fail validation, and cover this with focused tests.
