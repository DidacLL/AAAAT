...
from aaaat.agent_access import build_agent_work_item, submit_agent_task_result, task_capability
from aaaat.agent_work_service import claim_next_agent_work
...
class V1LifecycleConformanceTests(unittest.TestCase):
    ...
    def _candidature(self, **fields):
        with connect(self.storage) as conn:
            return create_candidature(
                ...
                raw_offer=fields.pop("raw_offer", "Example Co seeks a backend engineer."),
    ...
    def test_desktop_creation_and_actions_use_canonical_lifecycle_specs(self) -> None:
        ...
        self.assertEqual([(task["task_type"], task["context_hint"]) for task in tasks], [("field_inference", "candidature:extraction")])

        strategy = service.queue_candidature_action(created["id"], "regenerate_strategy")
        ...
        service.update_candidature_fields(created["id"], {"candidature_evaluation": "Strong fit"})
        with connect(self.storage) as conn:
    ...
    def test_lifecycle_work_packets_include_required_context_and_narrow_fields(self) -> None:
        ...
        self.assertIn("profile.skills", evaluation_work["input_context"]["profile_context"]["variables"])
        self.assertEqual(cv_work["input_context"]["candidature_evaluation"], "Strong fit")
        self.assertEqual(cv_work["input_context"]["role_strategy"], "Lead with reliable local systems.")
    ...
    def test_blocked_task_cannot_accept_results(self) -> None:
        with connect(self.storage) as conn:
            ...
            with self.assertRaisesRegex(ValueError, "not accepting results"):
                submit_agent_task_result(conn, capability, json.dumps({"cv_positioning": "Premature"}))
    ...
    def test_ingestion_releases_downstream_work_for_every_transport(self) -> None:
        with connect(self.storage) as conn:
            ...
            outcome = ingest_task_result(
                conn,
                capability,
                {"fields": {"candidature_evaluation": "Strong fit", "strengths": "Relevant experience", "risks_to_avoid": "Do not overstate scale", "valuation": 82}},
    ...
    def test_portable_export_claims_only_ready_work(self) -> None:
        ...
        outcome = export_candidature_task_bundle(self.storage, candidature["id"], bundle_path)
    ...
    def test_latest_explicit_desktop_request_is_claimed_before_background_work(self) -> None:
        with connect(self.storage) as conn:
            create_task(conn, "company_research", "Older background work", priority="high", context_hint="candidature:company_research", created_by="system")
            create_task(conn, "field_inference", "User requested review", priority="normal", context_hint="candidature:review", created_by="desktop_action")
    ...
    def test_profile_completion_does_not_disclose_existing_raw_values(self) -> None:
        with connect(self.storage) as conn:
            set_variable(conn, "profile.display_name", "Private Name", exposure="raw")
    ...
    def test_selected_candidature_can_render_local_cv_and_cover_letter(self) -> None:
        ...
        service.update_profile_variables(
            {
                ...
                "profile.summary.default": "Backend engineer focused on reliable local tools.",
            }
        )
        created = service.create_offer_first_candidature(
            "Example Co seeks a backend engineer.",
        ...
        service.update_candidature_fields(
            created["id"],
            {
                ...
                "cover_letter_material": "The role matches my experience building reliable local software.",
            },
        )
        ...
        self.assertEqual({item["artifact_type"] for item in service.list_candidature_artifacts(created["id"])}, {"cv", "cover_letter"})
