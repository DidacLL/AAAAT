    ...
    def test_profile_result_cannot_replace_an_existing_desktop_value(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with connect(tmp) as conn:
                ...
                self.assertNotIn(
                    "replace_existing",
                    json.dumps(work["response_format"]),
                )
                self.assertNotIn(
                    "User-owned name",
                    json.dumps(work["input_context"]),
                )
                self.assertIn(
        ...
        self.assertEqual(
            completed["profile_update"]["retained"],
            ["profile.display_name"],
        )
                        ...
                        json.dumps(
                            {
                                ...
                                "sources_checked": [str(index) for index in range(201)],
                            }
                        ),
                    )
