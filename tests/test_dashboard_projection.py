...
class DashboardProjectionTests(unittest.TestCase):
    def populated_projection(self, mode=Mode.FULL, view="smartView", selected_keyword="Python"):
        ...
        with connect(tmp.name) as conn:
            app = create_application(
                ...
                notes="Primary note text",
                keywords=["Python"],
                pitch="Reliable backend pitch",
            )
    ...
    def test_projection_can_be_built_without_rendering_templates(self):
        ...
        self.assertIn("permissions", projection)
    ...
    def test_welcome_and_user_projection_sections_are_structured(self):
        ...
        self.assertIn("setup_state", projection["welcome"])
        self.assertIn("open_task_summary", projection["welcome"])
    ...
    def test_smart_projection_preserves_selection_primary_note_and_keyword_context(self):
        ...
        self.assertEqual(smart["primary_note"]["source"], "applications.notes")
        self.assertTrue(smart["primary_note"]["editable"])
        self.assertEqual(smart["primary_note"]["history_count"], 1)
        self.assertEqual(smart["selected_keyword_context"]["term"], "Python")
        self.assertEqual(smart["selected_keyword_context"]["definition"], "Programming language.")
        self.assertEqual(smart["selected_keyword_context"]["selected_candidature"]["id"], app["id"])
    ...
    def test_detailed_projection_has_rows_columns_filters_and_selected_toolbox(self):
        ...
        self.assertGreaterEqual(len(detailed["rows"]), 2)
    ...
    def test_detailed_projection_general_toolbox_without_selected_row(self):
        projection = build_dashboard_projection(
            {"applications": [], "glossary": [], "profile_variables": {}, "missing_profile_variables": []},
            Mode.FULL,
            view="detailedView",
        )
        ...
        self.assertEqual(projection["detailed"]["rows"], [])
