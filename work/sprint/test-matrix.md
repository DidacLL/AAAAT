...
## Sprint 3 PR #3 Dashboard Correction Coverage

- No generated bytecode artifacts in PR scope: cleanup command removed `__pycache__` directories before verification; `compileall` can recreate local untracked bytecode.
- Compact application rows: `tests/test_dashboard_modes.py::DashboardModeTests.test_payload_and_mode_controls`, `tests/test_dashboard_modes.py::DashboardModeTests.test_application_list_focused_view_keyword_and_tabs_are_structured`
- Main dashboard has no large create/edit form or manual field dump: `tests/test_dashboard_modes.py::DashboardModeTests.test_payload_and_mode_controls`, `tests/test_dashboard_modes.py::DashboardModeTests.test_sparse_application_renders_cleanly`
- Selected application identity remains visible: `tests/test_dashboard_modes.py::DashboardModeTests.test_application_list_focused_view_keyword_and_tabs_are_structured`
- Keyword selection preserves selected app context: `tests/test_dashboard_modes.py::DashboardModeTests.test_application_list_focused_view_keyword_and_tabs_are_structured`
- Inspector tabs exist with Raw only in full mode: `tests/test_dashboard_modes.py::DashboardModeTests.test_payload_and_mode_controls`, `tests/test_dashboard_modes.py::DashboardModeTests.test_application_list_focused_view_keyword_and_tabs_are_structured`
- Inline editable fields use stable `data-inline-field` hooks in full mode and disappear in read-only mode: `tests/test_dashboard_modes.py::DashboardModeTests.test_payload_and_mode_controls`
- Raw-offer intake is dedicated, not a main-view form: `tests/test_dashboard_modes.py::DashboardModeTests.test_payload_and_mode_controls`, `aaaat.dashboard.render_raw_offer_intake_page`
- Review queue still works and shrinks after data is filled: `tests/test_review_queue.py::ReviewQueueTests.test_review_queue_includes_missing_fields_and_keyword_definitions_then_shrinks`
- Raw offer intake creates placeholder app, raw intake, and queue item: `tests/test_review_queue.py::ReviewQueueTests.test_raw_offer_intake_creates_placeholder_app_raw_intake_and_queue_item`
- Notes/source links render safely: `tests/test_dashboard_modes.py::DashboardModeTests.test_application_list_focused_view_keyword_and_tabs_are_structured`
- Static demo remains fake and non-writing: `tests/test_static_export.py::StaticExportTests.test_static_demo_uses_fake_payload_without_write_controls`
- CLI/API/MCP review queue still pass: `tests/test_cli_mcp.py`, `tests/test_server_api.py`, `python -B -m aaaat.cli mcp-validate`
